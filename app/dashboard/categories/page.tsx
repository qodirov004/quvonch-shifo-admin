...
import type { CategoryLanguage, Category, PaginatedResponse } from "@/lib/types"

export default function CategoriesPage() {
  const router = useRouter()
  const { user, token, loading, isAuthenticated } = useAuth()

  const [items, setItems] = useState<CategoryLanguage[]>([])
  const [filtered, setFiltered] = useState<CategoryLanguage[]>([])
  ...
  const [selectedItem, setSelectedItem] = useState<CategoryLanguage | null>(null)
  ...
  const fetchItems = async (currentPage: number) => {
    setPageLoading(true)
    try {
      const res: PaginatedResponse<CategoryLanguage> = await api.categories.getAll({ page: currentPage })
  ...
  const openEdit = (item: CategoryLanguage) => {
    setSelectedItem(item)
    setFormData({ name_uz: item.name_uz, name_ru: item.name_ru })
    setIsEditModalOpen(true)
  }

  const openView = (item: CategoryLanguage) => {
    setSelectedItem(item)
  ...
  const handleCreate = async () => {
    setFormLoading(true)
    try {
      await api.categories.create({ name_uz: formData.name_uz || "", name_ru: formData.name_ru || "" })
  ...
  const handleUpdate = async () => {
    ...
    setFormLoading(true)
    try {
      await api.categories.update(selectedItem.id, { name_uz: formData.name_uz || "", name_ru: formData.name_ru || "" })
  ...
  return (
    <DashboardLayout user={user}>
      <div className="space-y-4 sm:space-y-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-foreground mb-2">Kategoriyalar</h1>
          <p className="text-muted-foreground text-sm sm:text-base">Kategoriyalarni boshqarish</p>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Barcha Kategoriyalar ({filtered.length})</CardTitle>
          </CardHeader>
          <CardContent>
            {pageLoading ? (
      ...
      <Modal title="Kategoriya tahrirlash" open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <div className="space-y-4">
          <div className="space-y-2">
      ...
      <Modal title="Kategoriya" open={isViewModalOpen} onOpenChange={setIsViewModalOpen}>
        {selectedItem && (
          <div className="space-y-2">
            <div>
